...
def search_discussion(request):
    ...
    if request.method == 'POST':
        ...
        for i in range(0, 4):
            a.append(res[i])
        res_2 = RobotComment.objects.order_by('-date')
        b = []
        for i in range(0, 4):
            b.append(res_2[i])
        res_3 = RobotDiscuss.objects.order_by('-date')
        c = []
        for i in range(0, 4):
            c.append(res_3[i])
        status = True
        keyword = request.POST.get('search', '')
        res_2 = RobotDiscuss.objects.filter(title__contains=keyword)
        if res_2:
...
#         res = RobotDiscuss.objects.get(discuss_id=id)
#         comment =  RobotComment.objects.filter(discuss_id=id)
#         return render_to_response('chatcon.html', {'article': res, 'comment': comment, 'id': id, 'name': name, 'loginstatus': loginstatus})
#
# def issued(request):
#     name = ''
#     loginstatus = False
...
#                 content = request.POST.get('editor', 0)
#                 RobotDiscuss.objects.create(title=title, theme=theme, content=content, member_id=member,
...
#     except:
#         return HttpResponseRedirect('/login/?back=討論區發文')
#
